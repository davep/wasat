...
Protocol](gemini://geminiprotocol.net), relying only on `cryptography` for
...
- **Minimal Dependencies**: Relies only on `cryptography` for certificate
  management, otherwise using Python's standard library.
